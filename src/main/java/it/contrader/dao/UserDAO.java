...
public class UserDAO implements DAO<User> {
	...
	private final String QUERY_ALL = "SELECT * FROM user WHERE usertype <> 'SUPERADMIN'";
	private final String QUERY_CREATE = "INSERT INTO user (username, password, usertype, firstname, lastname, address, zipcode, city, province, email, phone) VALUES (?,?,?,?,?,?,?,?,?,?,?)";
	...
	public List<User> getAll() {
		...
		try {
			...
			while (resultSet.next()) {
				...
				user = new User( username,password, usertype, firstname, lastname, address, zipcode, city, province, email, phone);
				user.setId(id);
				usersList.add(user);
				user = new User(username,password, usertype, firstname, lastname, address, zipcode, city, province, email, phone);
				user.setId(id);
	...
	public boolean insert(User userToInsert) {
		...
		try {
			...
			preparedStatement.setString(4, userToInsert.getFirstname());
			preparedStatement.setString(5, userToInsert.getLastname());
			preparedStatement.setString(6, userToInsert.getAddress());
			preparedStatement.setString(7, userToInsert.getZipcode());
			preparedStatement.setString(8, userToInsert.getCity());
			preparedStatement.setString(9, userToInsert.getProvince());
			preparedStatement.setString(10, userToInsert.getEmail());
			preparedStatement.setString(11, userToInsert.getPhone());
	...
	public User read(int userId) {
		...
		try {
			...
			resultSet.next();
			String username,password, usertype, firstname, lastname, address, zipcode, city, province, email, phone;
			...
			User user = new User(username,password, usertype, firstname, lastname, address, zipcode, city, province, email, phone);
